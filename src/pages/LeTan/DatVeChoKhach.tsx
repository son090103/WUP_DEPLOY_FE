    ...
    for (let row = 1; row <= rows; row++) {
        ...
        columns.forEach((col: SeatColumn, colIndex: number) => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                seats.push({ id: `${floor}-${row}-${colIndex}-${i}`, floor: floor as 1 | 2, row, col: colIndex, status: bookedLabels.includes(label) ? "booked" : "available", label });
            }
        });
    }
    return seats;
        ...
        <div className="flex items-center gap-0 mb-6">
            {steps.map((s, i) => (
                <React.Fragment key={s.key}>
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all ${i === idx ? "bg-orange-500 text-white shadow-md" : i < idx ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-400"}`}>
...
const StaffBookingAll: React.FC = () => {
    ...
    useEffect(() => {
        const t = setTimeout(async () => {
            ...
            try {
                ...
                setDepSuggestions(Array.isArray(data) ? (data as SearchStop[]) : []);
    ...
    useEffect(() => {
        const t = setTimeout(async () => {
            ...
            try {
                ...
                const data: unknown = await res.json();
                setDestSuggestions(Array.isArray(data) ? (data as SearchStop[]) : []);
    ...
    const searchRoutes = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/customer/notcheck/search`, {
                ...
                body: JSON.stringify({ nodeId_start: departureId || null, nodeId_end: destinationId || null, name_start: !departureId ? departure : null, name_end: !destinationId ? destination : null, date: selectedDate }),
            });
            const data: unknown = await res.json();
            const list = Array.isArray(data) ? data : ((data as { data?: unknown })?.data ?? []);
            setRoutes(Array.isArray(list) ? (list as RouteItem[]) : []);
            setStep("trips");
    ...
    const selectRoute = async (route: RouteItem) => {
        ...
        try {
            ...
            const data: unknown = await res.json();
            const tripData = (data as { data?: unknown })?.data;
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE}/api/customer/notcheck/booked-seats`, {
            ...
            setBookedSeatLabels(Array.isArray(data) ? data : []);
        }).catch(console.error);
    ...
    const groupedSeats = useMemo(() => {
        ...
        currentSeats.forEach(s => {
            ...
            if (s.col === 0) g[s.row].LEFT.push(s); else g[s.row].RIGHT.push(s);
        });
        return g;
    }, [currentSeats]);
    ...
    const renderSeat = (seat: Seat) => {
        ...
        return (
            <button key={seat.id}
                onClick={() => { if (status === "booked") return; setSelectedSeats(prev => prev.includes(seat.id) ? prev.filter(x => x !== seat.id) : [...prev, seat.id]); }}
                disabled={status === "booked"}
                title={status === "booked" ? `Ghế ${seat.label} đã đặt` : `Ghế ${seat.label}`}
                className={`relative h-[32px] w-[62px] transition-all duration-300 ${status === "available" ? "hover:scale-110 cursor-pointer active:scale-95" : ""} ${status === "selected" ? "scale-110" : ""} ${status === "booked" ? "cursor-not-allowed opacity-60" : ""}`}>
    ...
    const canContinue = selectedSeats.length > 0 && !!selectedPickupId && !!selectedDropoffId &&
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const data: unknown = await res.json();
            const result = data as { success?: boolean; message?: string; data?: SuccessOrder };
        ...
        <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50 p-4 md:p-6" style={{ fontFamily: "'Nunito','Segoe UI',sans-serif" }}>
            <div className="max-w-5xl mx-auto">
                ...
                <div className="flex items-center gap-3 mb-5">
                    ...
                    {step !== "search" && (
                        <button onClick={resetAll} className="flex items-center gap-1.5 text-xs text-gray-500 border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 flex-shrink-0">
                ...
                {step === "search" && (
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div ref={depRef} style={{ position: "relative", zIndex: 200 }}>
                                ...
                                {depSuggestions.length > 0 && depRect && createPortal(
                                    <ul style={{ position: "fixed", top: depRect.bottom + 6, left: depRect.left, width: Math.max(depRect.width, 240), zIndex: 99999 }} className="bg-white rounded-2xl border border-gray-100 shadow-2xl overflow-hidden max-h-56 overflow-y-auto">
                                        {depSuggestions.map(s => (
                                            <li key={s._id} onMouseDown={e => { e.preventDefault(); setDeparture(s.province); setDepartureId(s._id); setDepSuggestions([]); }}
                            ...
                            <div ref={destRef} style={{ position: "relative", zIndex: 200 }}>
                                ...
                                {destSuggestions.length > 0 && destRect && createPortal(
                                    <ul style={{ position: "fixed", top: destRect.bottom + 6, left: destRect.left, width: Math.max(destRect.width, 240), zIndex: 99999 }} className="bg-white rounded-2xl border border-gray-100 shadow-2xl overflow-hidden max-h-56 overflow-y-auto">
                                        {destSuggestions.map(s => (
                                            <li key={s._id} onMouseDown={e => { e.preventDefault(); setDestination(s.province); setDestinationId(s._id); setDestSuggestions([]); }}
                                                className="flex items-center justify-between px-4 py-3 text-sm cursor-pointer hover:bg-orange-50">
                                                <div><p className="font-semibold text-gray-800">{s.name}</p><p className="text-xs text-gray-400">{s.province}</p></div>
                        ...
                        <button onClick={searchRoutes} disabled={loadingRoutes || !departure}
                            className={`w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${departure && !loadingRoutes ? "bg-gradient-to-r from-orange-400 to-orange-500 text-white hover:shadow-lg hover:scale-[1.01]" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>
                        ...
                        {routes.length === 0 ? (
                            ...
                            <div key={route._id} className={`bg-white rounded-2xl border-2 shadow-sm overflow-hidden transition-all ${selectedRoute?._id === route._id ? "border-orange-400 shadow-orange-100" : "border-gray-200 hover:border-orange-200"}`}>
                                <div className="p-4 flex items-center gap-4 flex-wrap">
                                    ...
                                    <button onClick={() => selectRoute(route)} className="bg-gradient-to-r from-orange-400 to-orange-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm hover:shadow-md transition-all active:scale-95 flex items-center gap-1.5 flex-shrink-0">
                                                        ...
                                                        <div className="flex items-center gap-2 flex-shrink-0">
                                                            {trip.time && trip.time.length > 0 && (
                                                                <button onClick={() => setOpenSchedule(isOpen ? null : trip._id)}
                                                                    className={`flex items-center gap-1 text-xs font-semibold px-3 py-2 rounded-lg border transition-all ${isOpen ? "bg-orange-50 border-orange-300 text-orange-600" : "bg-white border-gray-200 text-gray-500 hover:border-orange-200"}`}>
                                                                    <MapPin size={11} />Lịch trình{isOpen ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
                                                                </button>
                                                            )}
                                                            <button onClick={() => enterSeats(trip)} className="bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold px-5 py-2 rounded-xl text-sm hover:shadow-md transition-all active:scale-95 flex items-center gap-1.5">
                                                                    ...
                                                                    <div className="space-y-3">
                                                                        {[...trip.time].sort((a, b) => a.stop_order - b.stop_order).map((stop, idx) => (
                                                                            <div key={stop._id} className="flex items-start gap-3 pl-8 relative">
                                                                                <div className={`absolute left-0 w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black flex-shrink-0 ${idx === 0 ? "bg-green-500 text-white" : idx === (trip.time?.length ?? 0) - 1 ? "bg-orange-600 text-white" : "bg-white border-2 border-orange-300 text-orange-600"}`}>{idx + 1}</div>
                                                                                <div className="flex-1 min-w-0">
                            ...
                            <>
                                            ...
                                            <div className="space-y-2 lg:space-y-3">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <select value={selectedPickupId}
                                                        onChange={e => { const v = e.target.value; setSelectedPickupId(v); const found = pickupPoints.find(p => p._id === v); setSelectedPickupStopId(found?.stop_id._id ?? ""); }}
                                                ...
                                                {selectedPickupId && (
                                                    loadingPickupLocations
                                                        ? <div className="flex items-center gap-2 px-3.5 py-2.5 bg-green-50 border-2 border-green-100 rounded-xl"><div className="w-3.5 h-3.5 border-2 border-green-400 border-t-transparent rounded-full animate-spin flex-shrink-0" /><span className="text-xs text-green-600 italic">Đang tải vị trí chi tiết...</span></div>
                                                        : pickupLocationPoints.length > 0
                                                            ? <div className="space-y-1.5">
                                                                <div className="flex items-center gap-1.5 pl-0.5"><div className="w-1 h-1 rounded-full bg-green-400" /><span className="text-[11px] font-semibold text-green-600 uppercase tracking-wider">Vị trí cụ thể</span><span className="text-[10px] text-red-500 font-bold ml-0.5">*</span></div>
                                                                <div className="relative">
                                                                    <MapPin size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-green-400 pointer-events-none" />
                                                                    <select value={selectedPickupLocId} onChange={e => setSelectedPickupLocId(e.target.value)} className="w-full appearance-none pl-9 pr-9 py-2.5 bg-green-50 border-2 border-green-200 rounded-xl text-sm font-medium text-slate-700 cursor-pointer focus:outline-none focus:border-green-400 focus:ring-4 focus:ring-green-100">
                                                                        <option value="">-- Chọn vị trí cụ thể --</option>
                                                                        {pickupLocationPoints.map(lp => (<option key={lp._id} value={lp._id}>{lp.location_name}</option>))}
                                                                    </select><Chevron />
                                                                </div>
                                                                {selectedPickupLoc && <div className="flex items-start gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg"><MapPin size={12} className="text-green-500 mt-0.5 flex-shrink-0" /><span className="text-[11px] text-green-700 font-medium leading-relaxed">{selectedPickupLoc.location_name}</span></div>}
                                                            </div>
                                                            : null
                                                )}
                                            </div>
                                            ...
                                            <div className="space-y-2 lg:space-y-3">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <select value={selectedDropoffId}
                                                        onChange={e => { const v = e.target.value; setSelectedDropoffId(v); const found = dropoffPoints.find(p => p._id === v); setSelectedDropoffStopId(found?.stop_id._id ?? ""); }}
                                                ...
                                                {selectedDropoffId && (
                                                    loadingDropoffLocations
                                                        ? <div className="flex items-center gap-2 px-3.5 py-2.5 bg-orange-50 border-2 border-orange-100 rounded-xl"><div className="w-3.5 h-3.5 border-2 border-orange-400 border-t-transparent rounded-full animate-spin flex-shrink-0" /><span className="text-xs text-orange-600 italic">Đang tải vị trí chi tiết...</span></div>
                                                        : dropoffLocationPoints.length > 0
                                                            ? <div className="space-y-1.5">
                                                                <div className="flex items-center gap-1.5 pl-0.5"><div className="w-1 h-1 rounded-full bg-orange-400" /><span className="text-[11px] font-semibold text-orange-600 uppercase tracking-wider">Vị trí cụ thể</span><span className="text-[10px] text-red-500 font-bold ml-0.5">*</span></div>
                                                                <div className="relative">
                                                                    <Navigation size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-orange-400 pointer-events-none" />
                                                                    <select value={selectedDropoffLocId} onChange={e => setSelectedDropoffLocId(e.target.value)} className="w-full appearance-none pl-9 pr-9 py-2.5 bg-orange-50 border-2 border-orange-200 rounded-xl text-sm font-medium text-slate-700 cursor-pointer focus:outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100">
                                                                        <option value="">-- Chọn vị trí cụ thể --</option>
                                                                        {dropoffLocationPoints.map(lp => (<option key={lp._id} value={lp._id}>{lp.location_name}</option>))}
                                                                    </select><Chevron />
                                                                </div>
                                                                {selectedDropoffLoc && <div className="flex items-start gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg"><Navigation size={12} className="text-orange-500 mt-0.5 flex-shrink-0" /><span className="text-[11px] text-orange-700 font-medium leading-relaxed">{selectedDropoffLoc.location_name}</span></div>}
                                                            </div>
                                                            : null
                                                )}
                                            </div>
                                        ...
                                        {selectedPickup && selectedDropoff && (
                                            <div className="mx-4 lg:mx-6 mb-4 lg:mb-5 flex items-center gap-3 px-4 py-3 bg-white border-2 border-orange-200 rounded-xl shadow-sm">
                                                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                                    <div className="w-2 h-2 rounded-full bg-green-500 flex-shrink-0" />
                                                    <div className="min-w-0"><p className="text-xs font-bold text-green-700 truncate">{selectedPickup.stop_id.name}</p><p className="text-[10px] text-green-500 truncate">{selectedPickupLoc ? selectedPickupLoc.location_name : selectedPickup.stop_id.province}</p></div>
                                                </div>
                                                <div className="flex items-center gap-1 flex-shrink-0 w-16 lg:w-20">
                                                    <div className="flex-1 h-px bg-gradient-to-r from-green-400 to-orange-500" /><ArrowRight size={12} className="text-orange-500" />
                                                </div>
                                                <div className="flex items-center gap-1.5 min-w-0 flex-1 justify-end">
                                                    <div className="min-w-0 text-right"><p className="text-xs font-bold text-orange-700 truncate">{selectedDropoff.stop_id.name}</p><p className="text-[10px] text-orange-500 truncate">{selectedDropoffLoc ? selectedDropoffLoc.location_name : selectedDropoff.stop_id.province}</p></div>
                                                    <div className="w-2 h-2 rounded-full bg-orange-500 flex-shrink-0" />
                                                </div>
                                            </div>
                                        )}
                                ...
                                {selectedPickupStopId && selectedDropoffStopId && (
                                    <div className="px-3 lg:px-0">
                                        {loadingPrice
                                            ? <div className="flex items-center gap-2.5 px-5 py-3.5 bg-orange-50 border-2 border-orange-100 rounded-xl"><div className="w-4 h-4 border-2 border-orange-400 border-t-transparent rounded-full animate-spin flex-shrink-0" /><span className="text-sm text-orange-600 italic font-medium">Đang tính giá vé...</span></div>
                                            : ticketPrice !== null
                                                ? <div className="flex items-center justify-between gap-4 px-4 lg:px-5 py-3.5 lg:py-4 bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl shadow-lg">
                                                    <div className="flex items-center gap-3">
                                                        <div className="flex items-center justify-center w-8 lg:w-9 h-8 lg:h-9 rounded-full bg-white/20 flex-shrink-0"><span className="text-white text-base font-black">₫</span></div>
                                                        <div><p className="text-[10px] lg:text-[11px] font-bold text-orange-100 uppercase tracking-wider">Giá vé / 1 ghế</p><p className="text-xl lg:text-2xl font-black text-white leading-tight">{ticketPrice.toLocaleString("vi-VN")}₫</p></div>
                                                    </div>
                                                    {selectedSeats.length > 0 && <div className="text-right flex-shrink-0"><p className="text-[10px] lg:text-[11px] font-bold text-orange-100 uppercase tracking-wider">{selectedSeats.length} ghế</p><p className="text-lg lg:text-xl font-black text-white leading-tight">{totalPrice.toLocaleString("vi-VN")}₫</p></div>}
                                                </div>
                                                : null}
                                    </div>
                                )}
                                    ...
                                    <div className="flex gap-2 lg:gap-3 px-3 lg:px-0">
                                        {Array.from({ length: numFloors }, (_, i) => i + 1).map(f => (
                                            <button key={f} onClick={() => setSelectedFloor(f as 1 | 2)}
                                                className={`flex-1 py-2.5 lg:py-3 px-4 lg:px-6 rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-sm lg:text-base ${selectedFloor === f ? "bg-gradient-to-br from-orange-500 to-orange-600 text-white shadow-lg" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                                    ...
                                    <div className="hidden lg:block mt-6 bg-gradient-to-br from-orange-50 to-orange-100/50 border-2 border-orange-200 rounded-2xl p-6">
                                        <div className="flex items-center justify-between flex-wrap gap-4">
                                            <div>
                                                ...
                                                {!canContinue ? <p className="text-sm text-orange-600 font-medium flex items-center gap-1.5"><MapPin size={13} />{validationMessage()}</p> : <p className="text-sm text-slate-600">Vui lòng kiểm tra kỹ thông tin trước khi tiếp tục</p>}
                                            </div>
                                            {canContinue
                                                ? <button onClick={() => setStep("confirm")} className="bg-gradient-to-br from-orange-500 via-orange-600 to-orange-700 hover:from-orange-600 hover:to-orange-800 text-white px-8 py-4 rounded-xl font-bold shadow-xl hover:scale-105 transition-all flex items-center gap-2">Tiếp tục <ArrowRight size={20} /></button>
                                                : <button disabled className="bg-slate-200 text-slate-400 px-8 py-4 rounded-xl font-bold cursor-not-allowed flex items-center gap-2">Tiếp tục <ArrowRight size={20} /></button>}
                                        </div>
                                    </div>
                                )}
                            </>
                            ...
                            {selectedSeats.length > 0
                                ? <div className="px-4 py-3">
                                    <div className="flex items-center justify-between mb-2">
                                        <div><span className="text-xs text-slate-500 font-medium">Đã chọn </span><span className="text-sm font-black text-orange-600">{selectedSeats.length} ghế</span>{ticketPrice && <span className="text-xs text-slate-400 font-medium"> · {totalPrice.toLocaleString("vi-VN")}₫</span>}</div>
                                    ...
                                    {canContinue
                                        ? <button onClick={() => setStep("confirm")} className="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold py-3.5 rounded-2xl shadow-lg active:scale-95 transition-all text-base">Tiếp tục đặt vé <ArrowRight size={18} /></button>
                                        : <button disabled className="flex items-center justify-center gap-2 w-full bg-slate-200 text-slate-400 font-bold py-3.5 rounded-2xl cursor-not-allowed text-base">Tiếp tục đặt vé <ArrowRight size={18} /></button>}
                                </div>
                                : <div className="px-4 py-3.5 flex items-center justify-between">
                                    <div className="flex items-center gap-2"><Armchair size={18} className="text-orange-400" /><span className="text-sm font-semibold text-slate-600">Chọn ghế để tiếp tục</span></div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 pb-20 lg:pb-0">
                        <div className="flex-1 space-y-4">
                            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Nhập tên khách hàng" className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-300" />
                                    </div>
                                    <div>
                                        ...
                                        <input type="tel" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="0923..." className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-300" />
                                    </div>
                                </div>
                            </div>
                            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {([["CASH", "💵", "Tiền mặt"], ["CARD", "💳", "Thẻ/POS"], ["QR", "📱", "Mã QR"]] as const).map(([val, icon, label]) => (
                                        <button key={val} onClick={() => setPayMethod(val)} className={`flex flex-col items-center gap-1 py-3 px-2 rounded-xl border-2 transition-all ${payMethod === val ? "border-orange-400 bg-orange-50 shadow-sm" : "border-gray-200 bg-white hover:border-orange-200"}`}>
                            ...
                            <button onClick={handleConfirm} disabled={submitting}
                                className={`w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${!submitting ? "bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:shadow-lg hover:scale-[1.01]" : "bg-orange-300 text-white cursor-not-allowed"}`}>
